...
xml.rss version: "2.0", "xmlns:atom" => "http://www.w3.org/2005/Atom" do
  xml.channel do
    xml.title "Eventicus - Events in #{@city.name}"
    xml.description "Upcoming events in #{@city.name}, #{@city.country}"
    xml.link city_url(@city)
    xml.language I18n.locale.to_s
    xml.lastBuildDate @events.first&.updated_at&.to_fs(:rfc822) || Time.current.to_fs(:rfc822)
    xml.tag! "atom:link", href: feeds_events_url(@city.slug, format: :rss), rel: "self", type: "application/rss+xml"
